...
def test_skip_empty_and_temp_internal_sessions():
    ...
    assert should_skip_capture("", "answer", session_id="sess-1") == (True, "empty_turn")
    assert should_skip_capture("question", "", session_id="sess-1") == (True, "empty_turn")
...
def test_skip_only_trivial_short_recall():
    ...
    assert should_skip_capture("ok", "done", session_id="sess-1") == (True, "trivial_turn")
...
def test_does_not_skip_chinese_continue_next_step():
    for query in ["继续", "下一步", "继续下一步实验"]:
        ...
        assert should_skip_capture(query, "好的", session_id="sess-1") == (False, "")
